<template>
    <div class="register-container">
        ...
        <div class="register">
            ...
            <div class="content">
                <label>验证码:</label>
                <input placeholder="请输入验证码" v-model="code" name="password"
                       v-validate="{required :true,regex:/^[0-9]{4,6}$/}" :class="{invalid:errors.has('code')}"/>
...
export default {
    ...
    methods: {
        ...
        // 用户注册功能
        async userRegister() {
            const check_result = await this.$validator.validateAll()
            // 表单验证全部同通过后才能发送注册请求。
            if (check_result) {
                try {
